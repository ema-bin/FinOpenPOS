...
export async function PATCH(req: Request, { params }: RouteParams) {
  try {
    ...
    const { player1_id, player2_id, display_name, seed_number, notes, display_order, is_substitute, schedule_notes, schedule_restrictions_loaded, needs_same_day_close_matches } = body;
    ...
    if (player1_id !== undefined || player2_id !== undefined) {
      ...
      // Suma 13 damas: both women, both with damas category, sum >= 13
    ...
    if (needs_same_day_close_matches !== undefined) {
      updates.needs_same_day_close_matches = Boolean(needs_same_day_close_matches);
    }
